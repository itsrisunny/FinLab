  ...
  const [panError, setPanError] = useState({})
    ...
    const [aadharBackFile,setAadharBackFile] = useState("");
      ...
      useEffect(()=>{
		...
		if(!na && !pn && !ad && !pc && !afc && !abc && !Object.values(aadhaarError).includes(true) && !Object.values(panError).includes(true)){
		
			props.processData(name,panNumber,aadhar,panCard,aadharFrontCard,aadharBackCard);
	...
	//	processData(name,panNumber,aadhar,panCard,aadharFrontCard,aadharBackCard);
	  },[name,panNumber,aadhar,panCard,aadharFrontCard,aadharBackCard, aadhaarError, panError])
  ...
  if(validatePAN(e.target.value)){
    ...
    axios.post(API_URL+"api-auth/verify-pan-number-v3",{pan :e.target.value}).then((res) => {
      if(res?.data?.data!=undefined){
        ...
        setName(prevState => ({...prevState,[i]: response?.data?.result?.name}));
								...
								<div className="inner-employement">
									...
									<div className="form-group loan-in row">
										<div className="col-md-6 spacing">
											...
											<InputMask mask="aaaaa9999a" maskChar="" name={"pan"+i} className={panNumberError?"salary-input error":"salary-input"} value={panNumber[i]} placeholder="AZSPT5997H" onChange={(e)=>handlePanNumber(i,e.target.value)} onKeyUp={(e) => validateDirPanNumber(e, i)}/>
											{(panError[i] == false)?<><i className="fa fa-check" style={{"position": "absolute","color": "green"}}></i></>:""}
										</div>
										<div className="col-md-6 spacing">

											<label>Aadhaar Number</label>
											<InputMask mask="9999 9999 9999" maskChar="" name={"aadhar"+i} className={panNumberError?"salary-input error":"salary-input"} value={aadhar[i]} placeholder="9868 2341 8876" onChange={(e)=>handleAadharNumber(i,e.target.value)} onKeyUp={(e) => verifyAadharNo(e, i)}/>
											{(aadhaarError[i] == false)?<><i className="fa fa-check" style={{"position": "absolute","color": "green"}}></i></>:""}
										</div>
									</div>
									<div className="form-group loan-in row">
										<div className="col-md-12 spacing">
											<label>Name</label>
											<input type="text" name={"name"+i} className={panNumberError?"salary-input error":"salary-input"} value={name[i]} placeholder="Name" onChange={(e)=>{handelName(i,e.target.value)}} />
											{panNumberError && <small className="alertbox">Please fill here</small>}
										</div>
									</div>
											...
											<div className="filnameIcons">
												<div className="fileIc">
													<span className="imgWithName">
														<span className="imgIcons"><Icon.FiletypePng color={"#1962CD"} size={50}/></span>
														<span className="imageName">
														
															<span className={"vrtext"}>{(Object.keys( panCard).length && panCard[i]!="" )?((panCardFileName[i])):(panFile!=undefined && panFile !="")?panFile:"Upload Pan Card Image"}</span>
															<span className="vrtext message">* File Format: JPEG, PNG, and GIF; Max size: 2MB</span>
														</span>
													</span>
													{(Object.keys( panCard).length && panCard[i]!="" )?<>
													<span className="greenTick" onClick={e =>{removeData("panCard",i)}}><i className="fa fa-check"></i></span>
													</>:""}
												</div>
											</div>
											<div className="uploadbtnSize">
												<input type="file" onChange={(e) => handlePanCardDoc(i,e)} accept="image/png, image/jpeg, image/gif" style={{display:"none"}} id={"panCard"+i}/>
												<div className="uplodBtns" onClick={(e) => onBtnClick(i,"panCard")} >
											...
											<div className="filnameIcons">
												<div className="fileIc">
													<span className="imgWithName">
														<span className="imgIcons"><Icon.FiletypePng color={"#1962CD"} size={50}/></span>
														<span className="imageName">
															<span className={"vrtext"}>{(Object.keys(aadharFrontCard).length && aadharFrontCard[i]!=""  )?((aadharFrontFileName[i])):(aadharFrontFile!=undefined && aadharFrontFile !="")?aadharFrontFile:"Upload Aadhaar Front Image"}</span>
															<span className="vrtext message">* File Format: JPEG, PNG, and GIF; Max size: 2MB</span>
														</span>
													</span>
													{(Object.keys(aadharFrontCard).length && aadharFrontCard[i]!=""  )?<>
													<span className="greenTick" onClick={e =>{removeData("aadharFront",i)}}><i className="fa fa-check"></i></span></> : ""}
												</div>
											</div>
											<div className="uploadbtnSize">
												<input type="file" onChange={(e) => handleAadharFrontDoc(i,e)}  accept="image/png, image/jpeg, image/gif" style={{display:"none"}} id={"aadharFront"+i}/>
												<div className="uplodBtns"   onClick={(e) => onBtnClick(i,"aadharFront")}>
													Upload <i className="fa fa-cloud-upload"></i>
												</div>
											</div>
										</div>
											...
											<div className="filnameIcons">
												<div className="fileIc">
													<span className="imgWithName">
														<span className="imgIcons"><Icon.FiletypePng color={"#1962CD"} size={50}/></span>
														<span className="imageName">
															<span className={"vrtext"}>{(Object.keys(aadharBackCard).length && aadharBackCard[i]!="")?((aadharBackFileName[i])):(aadharBackFile!=undefined && aadharBackFile !="")?aadharBackFile:"Upload Aadhaar Back Image"}</span>
															<span className="vrtext message">* File Format: JPEG, PNG, and GIF; Max size: 2MB</span>
														</span>
													</span>
													{(Object.keys(aadharBackCard).length && aadharBackCard[i]!="")?<>
													<span className="greenTick" onClick={e =>{removeData("aadharBack",i)}}><i className="fa fa-check"></i></span></>:""}
												</div>
											</div>
											<div className="uploadbtnSize">
												<input type="file" onChange={(e) => handleAadharBackDoc(i,e)} accept="image/png, image/jpeg, image/gif" style={{display:"none"}} id = {"aadharBack"+i}/>
												<div className="uplodBtns" onClick={(e) => onBtnClick(i,"aadharBack")} >
													Upload <i className="fa fa-cloud-upload"></i>
												</div>
											</div>
										</div>
