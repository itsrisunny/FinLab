      ...
      const handleUploadDocument = () => {
		  ...
		  if(handleValidation()){
			setIsDisableBtn(true)
		    		...
		    		axios.post(API_URL+"admin/upload-bank-logo",formData).then((res) => {
...
const handleActiveIncative = (status, id) => {
    Swal.fire({
      title: status == 1?"Are you sure want to Incativate?":"Are you sure want to Ativate?",
      showDenyButton: true,
      ...
      denyButtonText: `No`
        ...
        if (result.isConfirmed) {
            axios.post(API_URL+"admin/change-bank-status",{id: id, status_state:status == 1?"0":"1"}).then((res) => {
                let response = res?.data;
                if(response?.status === 200){
                    getAllBankList();
                    Swal.fire({
                                            ...
                                            <div class="card mb-4">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div className="col-md-3">
                                                        <div class="mb-3">                                                        
                                                        <label className="col-form-label" for="basic-default-name">Bank Name</label>                                                        
                                                        <input type="text" className={(bankNameError)?"form-control errorlogin":"form-control"} placeholder="Enter Bank Name" value={bankName} onChange={handelBankName} />
                                                   ...
                                                   <div className="col-md-5"> 
                                                    ...
                                                    <label class="col-form-label" for="basic-default-name">Upload Logo</label>
                                                    <div class="col-sm-12">
                                                                ...
                                                                <span className="imgWithName">
                                                                   ...
                                                                   {(path=="")?<>
                                                                    <span className="imageName">
                                                                        <span title={bankLogoFileName} className={bankLogoError?"vrtext error":"vrtext"}>{bankLogo.length?((bankLogoFileName)):""}</span>
                                                                        {bankLogo.length?"":<>
                                                                        <span className={bankLogoError?"vrtext error":"vrtext message"}>{(!bankLogoError)?"* File Format: JPEG, PNG, and GIF; Max size: 2MB":"Upload Image"}</span></>}
                                                                        {bankLogoErrorMsg?<span className="vrtext message">{bankLogoErrorMsg}</span>:""}
                                                                        
                                                                    </span> </>:
                                                                     <span className="imageName">
                                                                         <span title={fileName} className={"vrtext"}>{fileName}</span>
                                                                     </span>
                                                                    }
                                                                </span>
                                                    ...
                                                    <label className="col-form-label col-md-12" style={{visibility:"hidden"}} for="basic-default-name">Bank </label>
                                                        <button className="btn btn-primary" onClick={handleUploadDocument} disabled={isDisableBtn}>
                                                    ...
                                                    /> Please Wait...</>:"Submit"
                                                        ...
                                                        {(editMode)?
                                                        <span>&nbsp;&nbsp;&nbsp;<button className="btn btn-primary" onClick={handleCancel} >Cancel</button>
                                                        </span>:""
                                                            }
                                                        </div>
                                                                ...
                                                                { (bankData)?
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <ul className="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
                                                                            ...
                                                                            <img src={API_URL+bk?.path} alt="Avatar" className="" style={{height:"80px"}} />
                                                                            </li>
                                                                        ...
                                                                        <button className="btn btn-view" onClick={(e) => handelEdit(bk)}>Edit</button>
                                                                         
                                                                         <button className={bk?.status?"btn btn-secondary":"btn btn-view"} style={{"margin-left":"2px"}} onClick={() => handleActiveIncative(bk?.status, bk?.id)}>{bk?.status?"In-active":"Active"}</button>
                                                                        </td>
                                                                    </tr>
