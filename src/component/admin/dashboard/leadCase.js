...
const LeadCase = () => {
      ...
      axios.post(API_URL+`admin/all-applied-loan-data`,jsonData).then((res) => {
        ...
        if(response?.status === 200){
          ...
          response?.data?.user_loan_detail?.map(v =>{
            ...
            v.appliedDate=moment(v.appliedDate,"YYYY-MM-DD hh:mm:ss").format("MM/DD/YYYY");
          })
    ...
    const getIndividualData =(caseId)=>{
      navigate('/admin/case-detail/'+caseId);
    }
   ...
   useEffect(() =>{
    getLoanData(0);
   },[loanTypeFilter,dateRangeFilter,sourceFilter,offerFilter])
