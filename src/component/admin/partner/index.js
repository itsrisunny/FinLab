...
const LeadCase = () => {
    ...
    // console.log(jsonData);
      ...
      axios.post(API_URL+`admin/all-partner-data`,jsonData).then((res) => {
  ...
  const updateStatus = (id,status) =>{
    ...
    axios.post(API_URL+`admin/update-partner_status`,jsonData).then((res) => {
        let response = res.data;
        if(response?.status === 200){
									...
									<div className="tableLeadCase">
                                      ...
                                      <tbody>
                                          ...
                                          <tr key={index} style={{ borderBottom: '1px solid #ddd' }}>
                                              ...
                                              <td className="table-body">{row.partner_id}</td>
                                              <td className="table-body">{row.email_id}</td>
                                              <td className="table-body">{row.mobile_number?<CurrencyFormat value={row.mobile_number} displayType={'text'} format="### ### ####" />:""}</td>
                                              <td className="table-body">{row.pan_card}</td>
                                              <td className="table-body">{row.gst}</td>
                                              <td className="table-body">{row.register_as}</td>
                                              <td className="table-body">{row.appliedDate}</td>
                                              <td className="table-body"> 
                                                <button className={(row.status)?"btn btn-active":"btn btn-delete"} onClick={() => {(row.status)?updateStatus(row.id,0):updateStatus(row.id,1)}}>{(row.status)?"✔":"✘"}</button>
                                              </td>
                                              <td className="table-body">
                                                <button className="btn btn-view" onClick={() => getIndividualData(row.id)}>View</button>
