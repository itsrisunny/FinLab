...
const appFunnelFunction = () => {
	axios.post(API_URL+`admin/dashboard-application-status`,{"partnerId":localStorage.getItem("partner_id")}).then((res) => {
		...
		setAllLeads(parseInt(res?.data?.data?.incompleteLeads ? res?.data?.data?.incompleteLeads : 0) + parseInt(res?.data?.data?.leads ? res?.data?.data?.leads : 0) + parseInt(res?.data?.data?.offeredLeads ? res?.data?.data?.offeredLeads :0) + parseInt(res?.data?.data?.closedLeads ? res?.data?.data?.closedLeads : 0) + parseInt(res?.data?.data?.rejectLeads ? res?.data?.data?.rejectLeads : 0))
	}).catch((e) => {
		console.log(e)
	});
}
...
const getLoanDataDateWise = () =>{
	axios.post(API_URL+`admin/dashboard-loan-amt-date-wise`,{"partnerId":localStorage.getItem("partner_id")}).then((res) => {
...
const getLoanDataPartner = () =>{
	axios.post(API_URL+`admin/dashboard-loan-amt`,{"partnerId":localStorage.getItem("partner_id")}).then((res) => {
	 	let response = res.data;
	 	if(response?.status === 200){
	 		...
	 		setTotalFunding(parseInt(response?.data?.appliedLoan?response?.data?.appliedLoan:0)+parseInt(response?.data?.approvedLoan?response?.data?.approvedLoan:0)+parseInt(response?.data?.disburstLoan?response?.data?.disburstLoan:0))
			/*
								...
								<div className="col-lg-6 col-md-6 col-sm-6">
									<div className="income-dashone fistSec">
										<div className="analytics-content">
											...
											<h6><span>{incompLead ? <CurrencyFormat value={incompLead} thousandSpacing={'2s'} displayType={'text'} thousandSeparator={true}  />:"0"}</span></h6>											
										</div>
									</div>
								</div>
								...
								<div className="col-lg-6 col-md-6 col-sm-6">
									<div className="income-dashone secondSec">
										<div className="analytics-content">
											...
											<h6><span>{lead ? <CurrencyFormat value={lead} thousandSpacing={'2s'} displayType={'text'} thousandSeparator={true}  />:"0"}</span></h6>
										</div>
									</div>
								</div>
								</div>
								...
								<div className="col-lg-6 col-md-6 col-sm-6 ">
									<div className="income-dashone thirdSec">
										<div className="analytics-content">
											...
											<h6><span>{offered ? <CurrencyFormat value={offered} thousandSpacing={'2s'} displayType={'text'} thousandSeparator={true}  />:"0"}</span></h6>
										</div>
									</div>
								</div>
								...
								<div className="col-lg-6 col-md-6 col-sm-6 ">
									<div className="income-dashone fourSec">
										<div className="analytics-content">
											...
											<h6><span>{close ? <CurrencyFormat value={close} thousandSpacing={'2s'} displayType={'text'} thousandSeparator={true}  />:"0"}</span></h6>
										</div>
									</div>
								</div>
								</div>
						...
						<div className="sectionTwos">
							<div className="row sectionAnotherKpi">
								...
								<div className="row">
									...
									<div className="income-dashone fistSec" style={{backgroundColor:"#506ffb",color:"white"}}>
										<div className="analytics-content">
										<h6>Total Applications Amount : <CurrencyFormat value={totalFunding?totalFunding:0} thousandSpacing={'2s'} displayType={'text'} thousandSeparator={true} prefix={"₹"}  /></h6>										
										</div>
									</div>
									</div>
								</div>
								...
								<div className="col-lg-6 col-md-6 col-sm-6">
									<div className="income-dashone fistSec">
										<div className="analytics-content">
											...
											<h6><span>{appliedAmount ? <CurrencyFormat value={appliedAmount} thousandSpacing={'2s'} displayType={'text'} thousandSeparator={true} prefix={"₹"}  />:"0"}</span></h6>
