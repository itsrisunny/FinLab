...
const LeadCase = () => {
    ...
    const expectedHeaders = ["Loan Purpose", "Sub Loan Purpose", "Loan Amount", "Duration", "Name", "Mobile Number", "Email ID", "PAN Number", "Date Of Birth", "Aadhaar Number", "Address 1", "Address 2", "Landmark", "PIN Code", "City, District", "State", "Gender", "Relationship status", "Active Loan", "EMI", "Monthly Salary", "Salary received via", "Company Name", "Tenure In Current Company", "Total Work Experience", "Official Email-id", "Serving Notice Period", "Working As/On"];
    ...
    // console.log(jsonData);
      ...
      axios.post(API_URL+`partners-admin/all-applied-loan-data-partner`,jsonData).then((res) => {
        ...
        if(response?.status === 200){
          ...
          response?.data?.user_loan_detail?.map(v =>{
            ...
            v.appliedDate=moment(v.appliedDate,"YYYY-MM-DD hh:mm:ss").format("DD/MM/YYYY");
          })
        ...
        const arrayOfObjects = arrNewValue.map(row => {
           if(row.length){
            ...
            keys.forEach((key, index) => {
                obj[(key.trim().toLowerCase().replace(regex,"")).replace(/\s+/g, '_')] = row[index];
            });
            return obj;
          }
        });
...
const uploadFileData = (datas) => {
  setLoader(true)
    ...
    axios.post(API_URL+`partners-admin/bulk-upload-case-leads`,jsonData).then((res) => {
 //   axios.post(API_URL+`bulk-upload/bulk-upload-case-leads`,jsonData).then((res) => {
        ...
        if(response?.status === 200){
         // toast.success(response?.message);   
          ...
          new_record.length && new_record.map((v, i) => {
            if(v.duration > 5){
              v.duration = "5+";
            }
            return v;
          ...
          let merged = [...response?.not_inserted_data, ...response?.new_records, ...response?.invalid_records];
...
const handelNewRecords = ()=>{
  ...
  axios.post(API_URL+`partners-admin/bulk-upload-case-leads-insert-data`,jsonData).then((res) => {
      let response = res.data;
      if(response?.status === 200){
        toast.success(response?.message);  
        dupRecords.map((v, i) => {
      ...
      <div className="mainContent">
        <div className="topHeadings">
          <h3>Lead cases</h3>
                ...
                <div> 
                {/* <span><label style={{"marginRight":"3px"}}>{fileName==null?"Select file to upload":fileName}</label>
                <label for="files" className="btn btn-success">Bulk Upload</label>
                <input id="files" type="file" accept=".csv" style={{"visibility":"hidden"}}  onChange={handleFileChange} /></span> */}
                <button className="btn btn-primary"style={{"marginRight":"3px"}} onClick={() => uploadDetails()}>Bulk Upload</button>
                 <span><Link to="/partners-admin/personal/add-case" className="btn btn-success">Create New</Link></span>
                 {/* <span><Link to={API_URL + 'templates/leadCasesBulkUpload.csv'} target="_blank" download="Bulk_upload_template" className="float-right" >Download Bulk upload template</Link></span> */}
                                      ...
                                      <tbody>
                                        {/* //   {filteredData.map((row, index) => ( */}
                                          ...
                                          <tr key={index} style={{ borderBottom: '1px solid #ddd' }}>
                                              <td className="table-body">{row.caseNumber}</td>
                                              <td className="table-body">{row.emailId}</td>
                                              <td className="table-body">{row.phoneNumber?<CurrencyFormat value={row.phoneNumber} displayType={'text'} format="### ### ####" />:""}</td>
                                              <td className="table-body">{row.loanAmountRequired ? <CurrencyFormat value={row.loanAmountRequired} thousandSpacing={'2s'} displayType={'text'} thousandSeparator={true} prefix={'₹'} />:""}</td>
                                              {/*<td className="table-body">{row.loanType}</td>*/}
                                              <td className="table-body">{row.appliedDate}</td>
                                              <td className="table-body">{row.offerAdded}</td>
                                              <td className="table-body">{row.source}</td>
                                              <td className="table-body">
                                              <button className="btn btn-view" onClick={() => getIndividualData(row.caseId)}>View</button>
            ...
            >
                ...
                <Modal.Body style={{ maxHeight: '500px', overflowX: 'auto'}}>
                    <div className="row add-bank-info">
                      ...
                      {!fileUploaded ? <>
                        <span><label style={{"marginRight":"3px"}}>{fileName==null?"Select file to upload":fileName}</label>
                        <label for="files" className="btn btn-success">Upload File</label>
                        <input id="files" type="file" accept=".csv" style={{"visibility":"hidden"}}  onChange={handleFileChange} /></span>                                            
                      </> : <>
                      <span>
                      <input id="files" type="file" accept=".csv" style={{"visibility":"hidden"}}  onChange={handleFileChange} /> </span></>}
                      {error?<span><label style={{"marginLeft": "10px","marginRight":"20px", "color": "red" }}>{error}</label></span>:""}
                          
                          {fileUploaded && (
                            <>
                            <span>
                            <label style={{"marginLeft": "10px","marginRight":"20px", "color": "black" }}>Total Records : {newRecords.length + dupRecords.length + invalidRecords.length}</label>
                            </span>
                            <span>
                            <label style={{ "marginLeft": "10px","marginRight":"20px", "color": "green" }}>New Records : {newRecords.length}</label>
                            </span>
                            <span>
                            <label style={{ "marginRight": "20px", "color": "red" }}>Duplicate Records : {dupRecords.length}</label>
                            </span>
                            <span>
                            <label style={{ "marginLeft": "10px","marginRight":"20px", "color": "blue" }}>Invalid Records : {invalidRecords.length}</label>
                            </span>
                            </>   
                          )}
                        ...
                        <div className="col-md-12 label-line-height">
                          {fileUploaded && (<center><span>List of records</span></center> )}
                                    ...
                                    <table style={{  width: "100%" }} className="table table-stripped">
                                      ...
                                      <tbody>
                                          ...
                                          <tr key={index} style={{ borderBottom: '1px solid #ddd', "backgroundColor":(row?.recordExists?"#ff000014":row?.validRecords?"#00ff0421":"") }}>
                                              <td className="table-body">{row.name}</td>
                                              <td className="table-body">{row?.phoneInvalid?<span style={{"color":"red"}}>{row.mobile_number}</span>:row.mobile_number}</td>
                                              <td className="table-body">{row?.emailInvalid?<span style={{"color":"red"}}>{row.email_id}</span>:row.email_id}</td>
                                              <td className="table-body">{row?.panInvalid?<span style={{"color":"red"}}>{row.pan_number}</span>:row.pan_number}</td>
                                              <td className="table-body">{row?.loanPurposeInvalid?<span style={{"color":"red"}}>{row.loan_purpose}</span>:row.loan_purpose}</td>   
                                              <td className="table-body">{row.sub_loan_purpose}</td>
                                              <td className="table-body">{row?.amountInvalid?<span style={{"color":"red"}}>{row.loan_amount}</span>:row.loan_amount}</td>
                ...
                <Modal.Footer>
                    ...
                    {fileUploaded ?
                     <Button variant="primary" className="submit-btn-modal" onClick={handelNewRecords} disabled={ newRecordsNo ? false:true } >Proceed with {newRecordsNo } {newRecordsNo > 1 ?"new Records" : "new Record"}</Button>
                     :
                     <></>}
                   </Modal.Footer>
            </Modal>
